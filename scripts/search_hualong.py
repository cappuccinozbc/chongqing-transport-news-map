...
class NewsProcessor:
    ...
    def search_tavily_site(self, site, days=14, max_results=20):
        """使用Tavily搜索指定网站"""
        print(f"🔍 使用Tavily搜索 {site} 的重庆交通新闻...")
    ...
    def process_results(self, results):
        ...
        for i, result in enumerate(results):
            ...
            # 过滤：必须包含交通相关关键词
            traffic_keywords = ['交通', '轨道', '地铁', '高铁', '公交', '道路', '高速', '桥梁', '隧道', '建设', '规划', '运营']
    ...
    def generate_markdown(self, news_list, site_name):
        ...
        md = f"""# 重庆交通新闻整理（{site_name}）

**更新时间：** {datetime.now().strftime('%Y-%m-%d %H:%M')}
**新闻数量：** {len(news_list)} 条
**数据来源：** {site_name} 最近14天的重庆交通建设、规划、运营相关新闻
...
"""
    ...
    def send_to_feishu(self, content):
        """保存到文件供手动发送"""
        output_file = "/tmp/chongqing_traffic_news_hualong.md"
